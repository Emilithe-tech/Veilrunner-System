...
const CORE_FIELDS = {
  ...
  ammunition: [["caliber", "Caliber", "bullseye"], ["ammoType", "Type", "crosshairs"], ["damage.modifier", "Damage modifier", "burst"]],
...
export function projectItemOverview(item, { action = null, loaded = null, effectiveSystem = null, isGM = false, localize = key => key } = {}) {
  ...
  const subtypeValue = system.classification?.category || system.weaponType || system.armorType || system.kind || system.category || "";
  ...
  const core = [...(CORE_FIELDS[item.type] ?? []), ...(physical ? PHYSICAL_FIELDS : [])].flatMap(([path, label, icon, unit]) => {
    ...
    return [{ label, value: `${value}${unit ? ` ${unit}` : ""}`, icon: `fa-${icon}` }];
  });
  if (action && !physical) {
  ...
  for (const persona of array(system.persona)) features.push({ label: persona, description: "", icon: "fa-user" });
  const damageValue = action?.damageOutcome?.available ? action.damageOutcome.formula : physical ? system.damage?.base : "";
  ...
  return {
    ...
    core, requirements, features, damage, condition, progression,
