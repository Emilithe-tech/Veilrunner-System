...
import { ARCHETYPE_SUMMARIES, sortPathEntries } from "../../data/path-options.mjs";
...
import { TALENT_TREE_CANVAS, talentTreeCatalog, talentTreePage, talentTreeRoot, skillPointCostForLevel } from "../../data/talent-tree.mjs";
import { ProgressionCanvasSession } from "../../data/progression/canvas-session.mjs";
import { ProgressionCanvasReview } from "../../data/progression/canvas-review.mjs";
import { ProgressionCanvasGesture } from "../../data/progression/canvas-gesture.mjs";
import { buildProgressionCreationDraft, buildProgressionLayoutDraft, buildProgressionPlacementDraft, buildProgressionRemovalDraft, buildProgressionConnectionChangeDraft } from "../../data/progression/canvas-draft-builder.mjs";
import { progressionCanvasOutcomeView } from "../../data/progression/canvas-presentation.mjs";
...
import { magicTreeAccess, purchasedTalentTreePage, schoolAccessBranchId, searchTalentTree, talentTreeRecords } from "./talent-tree-view.mjs";
import { CatalogIndex, CatalogProvider, registerCatalogInvalidation } from "./storefront/catalog-provider.mjs";
import { addToCart, cartItemCount, cartTotal, cartWeight, mergeCarts, normalizeCart, removeFromCart, setCartQuantity } from "./storefront/cart.mjs";
import { DEFAULT_CATALOG_QUERY, catalogFacets, paginateCatalog, queryCatalog, toggleCatalogSubtypes } from "./storefront/query.mjs";
import { buildStorefrontCommit, validateStorefrontTransaction } from "./storefront/transaction.mjs";
import { syncActorTalentTreeItems, validateTalentTreeItemSources } from "./talent-tree-items.mjs";
import { renderStorefrontBrowser, renderStorefrontContext } from "./storefront/view.mjs";
import { QualityCatalogProvider, qualityCatalogFacets, queryQualityCatalog, registerQualityCatalogInvalidation } from "./quality-catalog.mjs";
import { LanguageCatalogProvider, queryLanguageCatalog, registerLanguageCatalogInvalidation } from "./language-catalog.mjs";
import { renderBiographyPane, renderContactPane, renderIdentityDetails, renderIdentityOverview, renderLanguagePane } from "./identity-view.mjs";
import { renderTreeWebGL } from "./tree-webgl-renderer.mjs";
import {
  TREE_CONNECTION_ROUTES, TREE_NODE_SHAPES, TREE_NODE_SIZE_PRESETS, TREE_PATH_STATE_ORDER, TREE_ZOOM, anchoredTreeViewport, clampTreeNodeCenter, compileSharedTreeRoutes, computeSmartTreeRing,
...
const AUTHORING_UNAVAILABLE = "Talent-tree authoring is temporarily unavailable while canonical Progression transactions are being connected.";
const LEGACY_TREE_EDITOR_MUTATION_ACTIONS = new Set([
  "edit-tree-guidance",
  "save-tree-guidance",
  "toggle-tree-connect",
  "toggle-tree-move",
  ...
  "align-tree-horizontal",
  "align-tree-vertical",
  "toggle-align-to-first-node",
  "square-tree-nodes",
  "space-tree-horizontal",
  "space-tree-vertical",
  "preview-tree-ring",
  "place-tree-ring",
  "add-connection-waypoint",
  "remove-connection-waypoint",
  "toggle-waypoint-lock-x",
  "toggle-waypoint-lock-y",
...
const LEVEL_OPTIONS = Object.freeze([
  { level: 1,  label: "Nobody",      image: "/systems/veilrunner/assets/img/chargen/level-1.webp" },
  { level: 5,  label: "Beginner",    image: "/systems/veilrunner/assets/img/chargen/level-5.webp" },
  { level: 10, label: "Standard",    image: "/systems/veilrunner/assets/img/chargen/level-10.webp" },
  { level: 20, label: "Experienced", image: "/systems/veilrunner/assets/img/chargen/level-20.webp" },
  { level: 30, label: "Prime",       image: "/systems/veilrunner/assets/img/chargen/level-30.webp" },
  { level: 40, label: "Elite Tier",  image: "/systems/veilrunner/assets/img/chargen/level-40.webp" }
]);
...
function chargenCompendium(packName) {
  const packs = game.packs;
  const systemId = String(game.system?.id ?? "Veilrunner");
  const collection = `${systemId}.${packName}`;
  const direct = packs?.get(collection);
  if (direct) return direct;
  const expectedSystem = systemId.toLocaleLowerCase();
  const expectedName = String(packName).toLocaleLowerCase();
  const installed = typeof packs?.values === "function" ? [...packs.values()] : [];
  return installed.find(pack => {
    const metadata = pack.metadata ?? {};
    const packageName = String(metadata.packageName ?? pack.collection?.split(".")[0] ?? "").toLocaleLowerCase();
    const name = String(metadata.name ?? pack.collection?.split(".").at(-1) ?? "").toLocaleLowerCase();
    return packageName === expectedSystem && name === expectedName;
  }) ?? null;
}
...
export function openTalentTreeEditor({ page = "magic", nodeId = "", actor = null, authoringRuntime = null } = {}) {
  if (!game.user?.isGM) {
    ui.notifications.warn("Only a GM can inspect canonical Progression authoring data.");
    return null;
  }
  if (!authoringRuntime) {
    ui.notifications.error(AUTHORING_UNAVAILABLE);
    return null;
  }
  if (existingTalentTreeEditor) {
  ...
  const targetActor = actor
    ?? game.user?.character
    ?? game.actors?.contents?.find(candidate => candidate.type === "hero" && candidate.isOwner)
    ?? game.actors?.contents?.find(candidate => candidate.type === "hero")
    ?? null;
  if (!targetActor) {
    ui.notifications.error("A Hero is required to render the canonical Progression canvas.");
    return null;
  }
  const editor = new CharacterCreationOverlay(targetActor, { mode: "treeEditor", page, nodeId, authoringRuntime });
  existingTalentTreeEditor = editor;
  void editor.render();
  return editor;
}
...
class CharacterCreationOverlay {
  constructor(actor, { mode = "creation", page = "magic", nodeId = "", portraitEditor = null, authoringRuntime = null, planningLevel = null } = {}) {
    ...
    this.canonicalReview = new ProgressionCanvasReview({ session: this.authoringSession });
    this.canonicalCanvasMutationsEnabled = false;
    this.canonicalGestureTool = "inspect";
    this.canonicalGesture = new ProgressionCanvasGesture();
    this.canonicalGestureVisual = null;
    this.canonicalAuthoringEvidence = { state: this.authoringRuntime ? "idle" : "unavailable", picker: null, inspection: null, layoutPreview: null, removalPreview: null, error: "" };
    this.step = 0;
    this.treeCanvasOpen = false;
    this.treePage = page === "skills" ? "skills" : "magic";
    this.canonicalDraft = { open: false, type: this.treePage === "magic" ? "spell" : "skill", name: "", description: "" };
    this.canonicalGraphDraft = { x: null, y: null, targetId: "" };
    ...
    this.treePulsePlan = null;
    this.treePulseTimers = new Set();
    this.treePulseRunId = 0;
    this.treePulseFrame = 0;
    this.treePulseLastFrame = 0;
    this.treeViewportDetailFrame = 0;
    this.treeZoomSettleTimer = 0;
    ...
    this.loadedCompendiumResources = new Set();
  ...
  async render() {
    ...
    this.root.addEventListener("lostpointercapture", event => {
      if (this.canonicalGesture.state?.pointerId === event.pointerId) this.#cancelCanonicalGesture();
    });
    ...
    this.root.addEventListener("scroll", event => {
      ...
      this.#queueTreeViewportDetail(board);
    }, true);
    ...
    this.treeCatalogHook = Hooks.on("veilrunnerTalentTreeCatalogChanged", () => {
      ...
      if (this.isTreeEditor) {
        this.#cancelCanonicalGesture();
        this.canonicalReview.invalidate();
        void this.#refreshCanonicalAuthoringEvidence();
      }
    ...
    this.root.focus();
    this.#ensureStepCompendiumResources();
  }

  close({ renderSheet = false } = {}) {
    this.#cancelCanonicalGesture();
    this.canonicalReview.invalidate();
    this.canonicalAuthoringEvidenceRevision = (this.canonicalAuthoringEvidenceRevision ?? 0) + 1;
    if (this.treeSelectionTimer) window.clearTimeout(this.treeSelectionTimer);
    if (this.treeZoomSettleTimer) window.clearTimeout(this.treeZoomSettleTimer);
    if (this.treeViewportDetailFrame) window.cancelAnimationFrame(this.treeViewportDetailFrame);
    this.#stopTreePulseAnimation();
  ...
  #draw() {
    ...
    this.#queueTreeViewportDetail();
    if (this.pendingTreeCenterId) this.#centerTreeOnNode(this.pendingTreeCenterId);
    if (current.key === "talents") this.#startTreePulseAnimation();
  ...
  #ensureStepCompendiumResources() {
    if (this.isTreeEditor) return;
    const step = this.#steps[this.step]?.key;
    const resource = ["species", "origin", "background"].includes(step)
      ? "references"
      : step === "profession"
        ? "path"
        : step === "credits"
          ...
          : step === "qualitiesFlaws"
            ...
            : step === "identity"
              ? "languages"
              : "";
    if (!resource || this.loadedCompendiumResources.has(resource)) return;
    this.loadedCompendiumResources.add(resource);
    const load = resource === "references"
      ? () => this.#loadReferenceOptions()
      : resource === "path"
        ? () => Promise.all([this.#loadPathReferenceOptions(), this.#loadUniqueAbilityReferences()])
        : resource === "storefront"
          ? async () => { this.catalogRecords = await this.catalogIndex.records(); }
          : resource === "qualities"
            ? async () => { this.qualityCatalogRecords = await this.qualityCatalogProvider.records(); }
            : async () => { this.languageCatalogRecords = await this.languageCatalogProvider.records(); };
    void Promise.resolve().then(load).catch(error => {
      console.warn(`Veilrunner | Unable to load Character Creation ${resource} compendium resources.`, error);
    }).finally(() => {
      if (this.root.isConnected && this.#steps[this.step]?.key === step) this.#draw();
    });
  }
  ...
  #decorateTreeShapes() {
    ...
    this.#refreshTreeEditorNodeSelection();
    const world = this.root.querySelector(".vr-cc-tree-world");
    world?.classList.toggle("node-move-mode", this.nodeMoveMode);
  }

  #refreshTreeEditorNodeSelection() {
    if (!this.isTreeEditor) return;
    const world = this.root.querySelector(".vr-cc-tree-world");
    if (!world) return;
    for (const marker of world.querySelectorAll(".vr-cc-node-selection-marker")) marker.remove();
    for (const element of world.querySelectorAll("[data-tree-drag]")) {
      const selected = this.selectedTreeNodes.has(element.dataset.treeDrag);
      element.classList.toggle("multi-selected", selected);
      if (selected) this.#addTreeSelectionMarker(element, world);
    }
  }

  #refreshTreeEditorInspector() {
    if (!this.isTreeEditor) return;
    const inspector = this.root.querySelector(".vr-cc-tree-editor-inspector");
    if (inspector) inspector.outerHTML = this.#treeEditorInspector();
  }

  #refreshTalentTreeInspector() {
    if (this.isTreeEditor || this.#steps[this.step]?.key !== "talents") return;
    const inspector = this.root.querySelector(".vr-cc-info.vr-cc-tree-inspector");
    if (inspector) inspector.outerHTML = this.#talentInspector();
  }

  #refreshTalentTreePurchase(targetNodeId, targetNodeKind) {
    if (this.isTreeEditor || this.#steps[this.step]?.key !== "talents") return;
    for (const node of this.root.querySelectorAll(".vr-cc-root-node, .vr-cc-school-node, .vr-cc-practice-node, .vr-cc-spell-node")) {
      const nodeId = node.dataset.treeNodeId;
      const nodeKind = node.dataset.treeRoot ? "root" : node.dataset.treeSchool ? "school" : node.dataset.treePracticeCard ? "practice" : node.dataset.treeRank ? "spell" : "";
      if (nodeId !== String(targetNodeId ?? "") || nodeKind !== targetNodeKind) continue;
      const record = nodeKind === "root" ? { root: talentTreeRoot(this.treePage) } : this.#treeCatalogEntry(nodeId, nodeKind);
      if (!record) continue;
      let markup = "";
      if (nodeKind === "root") markup = this.#rootMarkup(record.root, this.#magicInvestment(talentTreePage(this.treePage)));
      else if (nodeKind === "school") markup = this.#schoolMarkup(record.school, false);
      else if (nodeKind === "practice") markup = this.#practiceMarkup(record.school, record.practice, Boolean(this.openPracticeId));
      else if (nodeKind === "spell") markup = this.#spellRow(record.school, record.practice, record.spell, true, record.practice.color || record.school.color);
      if (!markup) continue;
      const template = document.createElement("template");
      template.innerHTML = markup;
      const replacement = template.content.querySelector("[data-tree-node-id]");
      if (replacement) node.replaceWith(replacement);
    }
    this.#refreshTalentTreeConnections();
    for (const pool of this.root.querySelectorAll(".vr-cc-tree-pools > div")) {
      const label = pool.querySelector("span")?.textContent;
      const available = label === "Talent Points" ? this.#treeAvailable("talent") : label === "Skill Points" ? this.#treeAvailable("skill") : null;
      if (available !== null) pool.querySelector("strong")?.replaceChildren(String(available));
    }
    this.#refreshTalentTreeInspector();
  }

  #refreshTalentTreeConnections() {
    this.#stopTreePulseAnimation();
    const current = this.root.querySelector(".vr-cc-tree-world > .vr-cc-tree-links");
    if (!current) return;
    ...
    const visiblePractice = practiceEntry && this.treePurchasedOnly
      ? { ...practiceEntry.practice, spells: (practiceEntry.practice.spells ?? []).filter(spell => purchasedLeaves.has(spell.id)) }
      : practiceEntry?.practice;
    let schools = this.treePurchasedOnly ? purchasedTalentTreePage(talentTreePage(page), this.state.talentTree) : talentTreePage(page);
    if (page === "magic" && this.treePurchasedOnly) {
      const grantedSchool = talentTreePage(page).find(school => school.id === this.#magicAccess().schoolId);
      if (grantedSchool && !schools.some(school => school.id === grantedSchool.id)) schools = [...schools, { ...grantedSchool, practices: [] }];
    }
    const template = document.createElement("template");
    template.innerHTML = practiceEntry
      ? this.#practiceSpellWorld(practiceEntry.school, visiblePractice, page)
      : this.#treeWorld(schools, page);
    const replacement = template.content.querySelector(".vr-cc-tree-links");
    if (!replacement) return;
    current.replaceWith(replacement);
    this.#decorateTreeLineCrossings();
    this.#renderTreeWebGL();
    this.#startTreePulseAnimation();
  }

  #renderTreeWebGL(world = this.root.querySelector(".vr-cc-tree-world"), options = {}) {
    renderTreeWebGL(world, { zoom: this.treeZoom, ...options });
  }

  #treePulseBranches(routes) {
    const idOf = record => record?.node?.id ?? "";
    const keyOf = record => `${record.source?.kind ?? ""}:${idOf(record.source)}->${record.target?.kind ?? ""}:${idOf(record.target)}`;
    const outgoing = new Map();
    const incoming = new Map();
    for (const route of routes) {
      const sourceId = idOf(route.source);
      const targetId = idOf(route.target);
      if (!sourceId || !targetId) continue;
      if (!outgoing.has(sourceId)) outgoing.set(sourceId, []);
      if (!incoming.has(targetId)) incoming.set(targetId, []);
      outgoing.get(sourceId).push(route);
      incoming.get(targetId).push(route);
    }
    const starts = routes.filter(route => (incoming.get(idOf(route.source))?.length ?? 0) !== 1 || (outgoing.get(idOf(route.source))?.length ?? 0) !== 1);
    const used = new Set();
    const branches = [];
    const appendPath = (path, nextPath) => `${path} ${nextPath.replace(/^M\s+[-+\d.]+(?:,|\s+)[-+\d.]+\s*/i, "")}`;
    const build = first => {
      if (used.has(first)) return;
      let current = first;
      let path = first.path;
      let length = Math.max(1, Number(first.pulseLength) || 1);
      used.add(first);
      while (true) {
        const targetId = idOf(current.target);
        const next = outgoing.get(targetId) ?? [];
        if (next.length !== 1 || (incoming.get(targetId)?.length ?? 0) !== 1 || used.has(next[0])) break;
        current = next[0];
        used.add(current);
        path = appendPath(path, current.path);
        length += Math.max(1, Number(current.pulseLength) || 1);
      }
      branches.push({ ...first, path, pulseLength: length, key: keyOf(first) });
    };
    for (const route of starts) build(route);
    for (const route of routes) build(route);
    return branches;
  }

  #stopTreePulseAnimation() {
    this.treePulseRunId += 1;
    if (this.treePulseFrame) window.cancelAnimationFrame(this.treePulseFrame);
    this.treePulseFrame = 0;
    this.treePulseLastFrame = 0;
    for (const timer of this.treePulseTimers ?? []) window.clearTimeout(timer);
    this.treePulseTimers = new Set();
    for (const bead of this.root?.querySelectorAll?.("[data-runtime-tree-pulse], [data-runtime-tree-pulse-path]") ?? []) bead.remove();
  }

  #startTreePulseAnimation() {
    if (this.isTreeEditor || this.treeZoom < .75 || !this.treePulsePlan?.entries?.length || globalThis.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches) return;
    const layer = this.root.querySelector(".vr-cc-tree-pulse-layer");
    if (!layer) return;
    const runId = ++this.treePulseRunId;
    // SVG getPointAtLength forces geometry work. Sample each curve once, then
    // interpolate in JavaScript while animating so 50 cosmetic beads do not
    // repeatedly ask the SVG renderer to re-evaluate their paths.
    const sampleCount = 48;
    const runtime = this.treePulsePlan.entries.map(entry => {
      const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
      path.setAttribute("d", entry.path);
      path.setAttribute("visibility", "hidden");
      path.dataset.runtimeTreePulsePath = "";
      layer.append(path);
      const bead = document.createElementNS("http://www.w3.org/2000/svg", "circle");
      bead.setAttribute("class", `vr-cc-tree-link-pulse ${entry.classes}`.trim());
      bead.setAttribute("r", "4");
      bead.setAttribute("fill", entry.color);
      bead.setAttribute("style", entry.css);
      bead.setAttribute("aria-hidden", "true");
      bead.dataset.runtimeTreePulse = "";
      layer.append(bead);
      const length = path.getTotalLength();
      const samples = length
        ? Array.from({ length: sampleCount + 1 }, (_, index) => path.getPointAtLength(length * index / sampleCount))
        : [];
      return { ...entry, path, bead, bounds: path.getBBox(), length, samples };
    });
    // Glow filters look good for a handful of beads, but each one becomes a
    // separate expensive compositing region. Large purchased trees retain a
    // bright core/stroke while avoiding that GPU-heavy cosmetic effect.
    layer.classList.toggle("tree-pulse-lite", runtime.length > 12 || layer.closest(".tree-performance-mode"));
    const startedAt = performance.now();
    const frameInterval = 1000 / 24;
    const scheduleFrame = (delay = 0) => {
      const request = () => {
        if (runId === this.treePulseRunId && layer.isConnected) this.treePulseFrame = window.requestAnimationFrame(frame);
      };
      if (delay <= 1) return request();
      const timer = window.setTimeout(() => {
        this.treePulseTimers.delete(timer);
        request();
      }, delay);
      this.treePulseTimers.add(timer);
    };
    const frame = now => {
      if (runId !== this.treePulseRunId || !layer.isConnected) return;
      this.treePulseFrame = 0;
      if (document.hidden) {
        scheduleFrame(250);
        return;
      }
      this.treePulseLastFrame = now;
      const elapsed = (now - startedAt) % this.treePulsePlan.cycleMs;
      const viewport = this.#treePulseViewport(layer);
      for (const entry of runtime) {
        const localTime = elapsed - entry.delayMs;
        const outsideRoute = viewport && (entry.bounds.x + entry.bounds.width < viewport.left || entry.bounds.x > viewport.right || entry.bounds.y + entry.bounds.height < viewport.top || entry.bounds.y > viewport.bottom);
        if (outsideRoute || localTime < 0 || localTime > entry.durationMs || !entry.length || !entry.durationMs) {
          entry.bead.setAttribute("visibility", "hidden");
          continue;
        }
        const progress = localTime / entry.durationMs;
        const samplePosition = Math.min(entry.samples.length - 1, Math.max(0, progress * (entry.samples.length - 1)));
        const sampleIndex = Math.floor(samplePosition);
        const sampleBlend = samplePosition - sampleIndex;
        const from = entry.samples[sampleIndex];
        const to = entry.samples[Math.min(sampleIndex + 1, entry.samples.length - 1)];
        const point = {
          x: from.x + (to.x - from.x) * sampleBlend,
          y: from.y + (to.y - from.y) * sampleBlend
        };
        if (viewport && (point.x < viewport.left || point.x > viewport.right || point.y < viewport.top || point.y > viewport.bottom)) {
          entry.bead.setAttribute("visibility", "hidden");
          continue;
        }
        entry.bead.setAttribute("cx", point.x);
        entry.bead.setAttribute("cy", point.y);
        entry.bead.setAttribute("visibility", "visible");
      }
      // Do not wake up on every display refresh only to skip work. One
      // requestAnimationFrame is scheduled for each 24 FPS animation tick.
      scheduleFrame(Math.max(0, frameInterval - (performance.now() - now)));
    };
    scheduleFrame();
  }

  #treePulseViewport(layer) {
    const world = layer.closest(".vr-cc-tree-world");
    const board = world?.closest(".vr-cc-tree-board");
    if (!world || !board) return null;
    const originX = Number.parseFloat(world.style.left) || 0;
    const originY = Number.parseFloat(world.style.top) || 0;
    return {
      left: (board.scrollLeft - originX) / this.treeZoom,
      right: (board.scrollLeft + board.clientWidth - originX) / this.treeZoom,
      top: (board.scrollTop - originY) / this.treeZoom,
      bottom: (board.scrollTop + board.clientHeight - originY) / this.treeZoom
    };
  }
  ...
  #decorateTreeWaypoints() {
    if (!this.isTreeEditor || !this.canonicalCanvasMutationsEnabled || !game.user.isGM || !this.connectionTool.active || !this.connectionTool.selection) return;
    const world = this.root.querySelector(".vr-cc-tree-world");
  ...
  #decorateConnectionSockets() {
    if (!this.connectionTool.active && this.canonicalGestureTool !== "connect") return;
    const world = this.root.querySelector(".vr-cc-tree-world");
    if (!world) return;
  ...
  #pathRecords() {
    if (!this.pathCatalogLoaded) return [];
  ...
  #syncPathReferences() {
    ...
    const selected = {
      archetype: this.#pathEntry("archetype", this.state.archetype),
      profession: this.#pathEntry("profession", this.state.profession, { archetype: this.state.archetype }),
      discipline: this.#pathEntry("discipline", this.state.discipline, { profession: this.state.profession })
    };
    for (const [kind, entry] of Object.entries(selected)) {
      this.state.references[kind] = entry?.uuid ?? "";
      this.state.references[kind + "DefinitionId"] = entry?.definitionId ?? "";
    }
  }
  ...
  #professionStep() {
    ...
    const archetypeNames = (this.pathReferenceOptions.archetype ?? []).map(entry => entry.name);
  ...
  async #loadReferenceOptions() {
    const entries = await Promise.all(Object.keys(REFERENCE_PACKS).map(async key => {
      try {
        const reader = new CanonicalDefinitionReader([key], { capability: "chargenSelectable" });
        const pack = reader.packs[0];
        const index = await reader.documents();
        ...
        return [key, index.filter(entry => entry.type === key).map(entry => ({
          ...
          definitionId: entry.system.definitionId,
          ...
          category: entry.folder?.name ?? folders.get(entry.folder) ?? key
  ...
  async #loadPathReferenceOptions() {
    const definitions = [
      ["archetype", "archetypes", ["system.definitionId", "system.summary", "system.description"]],
      ["profession", "professions", ["system.definitionId", "system.archetypeId", "system.archetype", "system.summary", "system.pageImage", "system.description"]],
    ...
    for (const [kind, packName, fields] of definitions) {
      try {
        const reader = new CanonicalDefinitionReader([kind], { capability: "chargenSelectable" });
        const pack = reader.packs[0];
        const index = await reader.documents();
  ...
  async #loadUniqueAbilityReferences() {
    ...
    for (const packName of UNIQUE_ABILITY_PACKS) {
      const pack = chargenCompendium(packName);
      if (!pack) continue;
      try {
        ...
        console.warn(`Veilrunner | Unable to load ${packName} unique abilities.`, error);
      }
    }
    ...
    return `<div class="vr-cc-reference-cards vr-cc-level-cards vr-cc-reference-cards-no-intro">
      <section class="vr-cc-reference-card-category"><h2>Starting Level</h2><div class="vr-cc-reference-card-options">
        ${LEVEL_OPTIONS.map(option => `<button type="button" class="vr-cc-reference-card vr-cc-full-art-choice-card vr-cc-level-full-art-card ${option.level === selected ? "active" : ""}" data-level-choice="${option.level}" aria-pressed="${option.level === selected}">
          <span class="vr-cc-choice-image vr-cc-selection-art" aria-hidden="true"><img src="${option.image}" alt="" /></span><span class="vr-cc-choice-name vr-cc-level-choice-name"><span>Level ${option.level}</span><small>${escape(option.label)}</small></span>
        </button>`).join("")}
          <label class="vr-cc-level-custom vr-cc-full-art-choice-card vr-cc-level-full-art-card ${selected && !LEVEL_OPTIONS.some(option => option.level === selected) ? "active" : ""}">
            <span class="vr-cc-choice-image vr-cc-selection-art" aria-hidden="true">
              <img src="systems/veilrunner/assets/img/chargen/custom-level.webp" alt="" />
            </span>
            <span class="vr-cc-choice-name vr-cc-level-choice-name">
              <span>Custom Level</span>
              <small>Enter a level</small>
              <input
                name="startingLevel"
                type="number"
                min="1"
                step="1"
                value="${selected && !LEVEL_OPTIONS.some(option => option.level === selected) ? selected : ""}"
                placeholder="Level"
              />
            </span>
          </label>
  ...
  #selectedReference(key) {
    const definitionId = this.state.references?.[key + "DefinitionId"];
    if (definitionId) return (this.referenceOptions[key] ?? []).find(entry => entry.definitionId === definitionId) ?? null;
  ...
  #talentHeader(current) {
    ...
    const testSpellButton = game.user.isGM && this.treePage === "magic" ? `<button type="button" class="vr-cc-btn" data-action="test-buy-all-spells" title="Testing only: purchase every Magic Practice and max every Spell without spending points"><i class="fa-solid fa-flask"></i><span>Test: Buy All Spells</span></button>` : "";
  ...
  #treeEditorHeader() {
    ...
    return `<header class="vr-cc-header vr-cc-talent-header vr-cc-editor-toolbar">
      ...
      <div class="vr-cc-editor-tool-modes" role="group" aria-label="Canvas mode">${[["inspect", "Inspect", "fa-magnifying-glass"], ["move", "Move preview", "fa-up-down-left-right"], ["connect", "Connect preview", "fa-link"]].map(([tool, label, icon]) => `<button type="button" class="vr-cc-btn ${this.canonicalGestureTool === tool ? "active" : ""}" data-action="set-canonical-gesture-tool" data-canonical-tool="${tool}" aria-pressed="${this.canonicalGestureTool === tool}" ${this.canonicalReview.state.busy || !this.authoringSession ? "disabled" : ""}><i class="fa-solid ${icon}"></i><span>${label}</span></button>`).join("")}<span class="vr-cc-canonical-readonly"><i class="fa-solid fa-shield-halved"></i> Read-only canonical session</span></div>
      <div class="vr-cc-tree-author-actions"><button type="button" class="vr-cc-btn ${this.canonicalDraft.open ? "active" : ""}" data-action="toggle-canonical-draft" aria-expanded="${this.canonicalDraft.open}"><i class="fa-solid fa-file-circle-plus"></i><span>Preview Draft</span></button><button type="button" class="vr-cc-btn" data-action="refresh-canonical-authoring"><i class="fa-solid fa-rotate"></i><span>Refresh</span></button><button type="button" class="vr-cc-btn vr-cc-inspector-toggle" data-action="toggle-tree-editor-inspector" aria-expanded="${this.treeEditorInspectorOpen}"><i class="fa-solid fa-sliders"></i><span>Inspector</span></button></div>
    </header>`;
  }
  ...
  #canonicalGraphNodeId(record) {
    if (!record?.id) return "";
    const prefix = record.kind === "root" ? "root" : record.kind === "school" ? "school" : record.kind === "practice" ? "practice" : record.kind === "spell" ? (this.treePage === "skills" ? "skill" : "spell") : "";
    return prefix ? `${prefix}:${record.id}` : "";
  }

  #canonicalDraftMarkup() {
    if (!this.canonicalDraft.open) return "";
    const draft = { ...this.canonicalDraft, ...this.canonicalReview.state };
    const record = this.#treeEditorSelectedRecords()[0] ?? null;
    const parentNodeId = this.#canonicalGraphNodeId(record);
    const parentName = record?.name ?? record?.node?.name ?? "No parent selected";
    const creation = !draft.built || draft.built.intent.operation === "create-content";
    const graphOperation = draft.built?.intent.operation;
    const graphTitle = graphOperation === "place-existing" ? "Preview existing placement" : graphOperation === "update-layout" ? "Preview node movement" : graphOperation === "replace-connections" ? "Preview connection changes" : "Preview graph-only removal";
    const graphHelp = graphOperation === "place-existing" ? `Link a canonical definition below ${parentName}. The existing content is not edited.`
      : graphOperation === "update-layout" ? "Move only the selected graph position. Canonical definitions and Actor Items are preserved."
        : graphOperation === "replace-connections" ? "Review the exact connection changes. Other edges retain their full authored routing and style."
          : "Remove this graph placement and its incident edges only. Canonical content and Actor Items are preserved.";
    const contentType = this.treePage === "magic" ? "spell" : "skill";
    const preview = draft.preview;
    const presented = draft.outcome ?? preview;
    const outcome = presented ? progressionCanvasOutcomeView({
      ...presented,
      ...(presented.preview ?? {}),
      definitionId: presented.definitionId ?? draft.built?.definitionId ?? presented.preview?.definitionId,
      nodeId: presented.nodeId ?? draft.built?.nodeId ?? presented.preview?.nodeId
    }) : null;
    const confirmation = this.canonicalReview.confirmation();
    const outcomeDetails = outcome ? `${outcome.identities.map(entry => `<span><b>${escape(entry.label)}:</b> ${escape(entry.value)}</span>`).join("")}${outcome.steps.length ? `<small>Steps: ${outcome.steps.map(entry => escape([entry.step, entry.status].filter(Boolean).join(" - "))).join("; ")}</small>` : ""}${outcome.rollback.length ? `<small>Rollback: ${outcome.rollback.map(entry => escape([entry.step, entry.status, entry.message].filter(Boolean).join(" - "))).join("; ")}</small>` : ""}${outcome.code || outcome.receipt || outcome.lockSession ? `<details><summary>Transaction and recovery details</summary><pre>${escape(JSON.stringify({ code: outcome.code, stage: outcome.stage, issues: outcome.issues, receipt: outcome.receipt, lockSession: outcome.lockSession, causes: outcome.causes }, null, 2))}</pre></details>` : ""}` : "";
    const previewMarkup = presented
      ? ["previewed", "committed"].includes(presented.state)
        ? `<div class="vr-cc-canonical-draft-result valid" role="status" aria-live="polite" tabindex="-1"><strong>${escape(outcome.heading)}</strong>${outcomeDetails}<small>${escape(outcome.message)}</small></div>`
        : `<div class="vr-cc-canonical-draft-result refused" role="status" aria-live="assertive" tabindex="-1"><strong>${escape(outcome.heading)}</strong>${outcomeDetails}<span>${escape(presented.issues?.[0]?.message ?? outcome.message ?? presented.code ?? "Canonical validation failed.")}</span></div>`
      : `<p class="vr-cc-field-help" role="status" aria-live="polite">${draft.built ? "Validating graph preview..." : "Preview uses the exact canonical transaction candidate without opening a pack lock."}</p>`;
    const confirmationMarkup = !draft.outcome && confirmation.stage === "review"
      ? `<section class="vr-cc-canonical-confirmation review" aria-label="Canonical write review"><strong>Persistent write review required</strong><p>Inspect the exact documents and graph changes before opening confirmation.</p><button type="button" class="vr-cc-btn" data-action="review-canonical-draft"><i class="fa-solid fa-clipboard-check"></i><span>Review Mutation</span></button></section>`
      : !draft.outcome && confirmation.stage === "confirmation"
        ? `<section class="vr-cc-canonical-confirmation" role="region" aria-label="Canonical write confirmation"><header><small>Step 2 of 2</small><strong>Confirm persistent canonical write</strong></header><ul>${confirmation.changes.map(change => `<li>${escape(change)}</li>`).join("")}</ul><p><strong>Pack scope:</strong> ${confirmation.collections.map(collection => escape(collection)).join(" + ")}</p><label><input type="checkbox" name="canonicalDraft.acknowledged" ${confirmation.acknowledged ? "checked" : ""} /> I understand this would persist canonical compendium and graph changes.</label><p class="vr-cc-canonical-confirmation-status" role="status" aria-live="polite">${escape(confirmation.blockingReason || "Confirmation is complete.")}</p><footer><button type="button" class="vr-cc-btn" data-action="cancel-canonical-draft-confirmation">Back</button><button type="button" class="vr-cc-btn primary" data-action="commit-canonical-draft" ${confirmation.commitEnabled ? "" : "disabled"}><i class="fa-solid fa-${confirmation.commitEnabled ? "floppy-disk" : "lock"}"></i><span>${confirmation.commitEnabled ? "Commit canonical write" : "Commit unavailable"}</span></button></footer></section>`
        : "";
    return `<form class="vr-cc-canonical-draft" data-canonical-draft-form aria-busy="${draft.busy}"><fieldset ${draft.busy ? "disabled" : ""}>
      <header><div><small>Canonical draft</small><strong>${creation ? "Preview new content" : graphTitle}</strong></div><button type="button" class="vr-cc-icon" data-action="toggle-canonical-draft" aria-label="Close canonical draft preview"><i class="fa-solid fa-xmark"></i></button></header>
      ${creation ? `
      <p>Select a School or group before drafting a Practice; select a Practice before drafting a ${this.treePage === "magic" ? "Spell" : "Skill"}.</p>
      <label>Content type<select name="canonicalDraft.type"><option value="practice" ${draft.type === "practice" ? "selected" : ""}>Practice</option><option value="${contentType}" ${draft.type === contentType ? "selected" : ""}>${this.treePage === "magic" ? "Spell" : "Skill"}</option></select></label>
      <label>Name<input name="canonicalDraft.name" value="${escape(draft.name)}" placeholder="Canonical display name" autocomplete="off" /></label>
      <label>Description<textarea name="canonicalDraft.description" rows="3" placeholder="Short canonical description">${escape(draft.description)}</textarea></label>
      <dl><div><dt>Placement parent</dt><dd>${escape(parentName)}</dd></div><div><dt>Internal ID entry</dt><dd>Automatic</dd></div></dl>` : `<p>${escape(graphHelp)}</p>`}
      <div data-canonical-review-evidence>${previewMarkup}${confirmationMarkup}</div>
      <footer><button type="button" class="vr-cc-btn" data-action="clear-canonical-draft">Clear</button>${creation ? `<button type="button" class="vr-cc-btn primary" data-action="preview-canonical-draft" ${parentNodeId ? "" : "disabled"}><i class="fa-solid fa-circle-check"></i><span>Validate Preview</span></button>` : ""}</footer>
    </fieldset></form>`;
  }

  async #previewCanonicalDraft() {
    if (this.canonicalReview.state.busy) return;
    const record = this.#treeEditorSelectedRecords()[0] ?? null;
    const type = this.canonicalDraft.type;
    const schoolNode = record?.kind === "school" ? record.node : record?.school;
    const practiceNode = record?.kind === "practice" ? record.node : record?.practice;
    try {
      const built = buildProgressionCreationDraft({
        page: this.treePage,
        type,
        name: this.canonicalDraft.name,
        school: schoolNode ? { id: schoolNode.id, name: schoolNode.name } : null,
        practice: practiceNode ? { id: practiceNode.id, name: practiceNode.name } : null,
        practiceId: type === "practice" ? this.canonicalDraft.name : "",
        position: { x: number(record?.node?.x) + 160, y: number(record?.node?.y) + 160 },
        parentNodeId: this.#canonicalGraphNodeId(record),
        definition: { description: this.canonicalDraft.description, summary: this.canonicalDraft.description }
      });
      if (!await this.canonicalReview.preview(built)) return;
    } catch (error) {
      this.canonicalReview.refuse(error);
    }
    this.#refreshTreeEditorInspector();
  }

  async #previewCanonicalPlacementOrRemoval({ definitionId = "", remove = false } = {}) {
    if (this.canonicalReview.state.busy) return;
    this.canonicalDraft.open = true;
    this.treeEditorInspectorOpen = true;
    const record = this.#treeEditorSelectedRecords()[0] ?? null;
    try {
      const built = remove ? buildProgressionRemovalDraft({ page: this.treePage, nodeId: this.#canonicalGraphNodeId(record) })
        : buildProgressionPlacementDraft({
          page: this.treePage,
          definition: this.canonicalAuthoringEvidence.picker?.items?.find(item => item.definitionId === definitionId),
          parentNodeId: this.#canonicalGraphNodeId(record),
          position: { x: number(record?.node?.x) + 160, y: number(record?.node?.y) + 160 }
        });
      if (!await this.canonicalReview.preview(built)) return;
    ...
    this.#refreshTreeEditorInspector();
    this.root.querySelector(".vr-cc-canonical-draft-result[tabindex]")?.focus({ preventScroll: false });
  }

  async #refreshCanonicalAuthoringEvidence() {
    if (!this.isTreeEditor || !this.authoringRuntime) return;
    this.#cancelCanonicalGesture();
    this.canonicalReview.invalidate();
    this.canonicalGraphDraft = { x: null, y: null, targetId: "" };
    const revision = (this.canonicalAuthoringEvidenceRevision ?? 0) + 1;
    this.canonicalAuthoringEvidenceRevision = revision;
    this.canonicalAuthoringEvidence = { ...this.canonicalAuthoringEvidence, state: "loading", error: "" };
    this.#refreshTreeEditorInspector();
    try {
      const record = this.#treeEditorSelectedRecords()[0] ?? null;
      const nodeId = this.#canonicalGraphNodeId(record);
      const [picker, inspection, layoutPreview, removalPreview] = await Promise.all([
        this.authoringRuntime.listDefinitions({
          page: this.treePage,
          query: this.treeSearch,
          types: this.treePage === "magic" ? ["practice", "spell"] : ["practice", "skill"],
          limit: 20
        }),
        nodeId ? this.authoringRuntime.inspectNode({ page: this.treePage, nodeId }) : null,
        nodeId ? this.authoringSession.preview(buildProgressionLayoutDraft({
          page: this.treePage,
          positions: [{ nodeId, position: { x: number(record?.node?.x), y: number(record?.node?.y) } }]
        }).intent) : null,
        nodeId ? this.authoringSession.preview({
          operation: "remove-node",
          page: this.treePage,
          payload: { nodeId }
        }) : null
      ]);
      if (revision !== this.canonicalAuthoringEvidenceRevision || !this.root.isConnected) return;
      this.canonicalAuthoringEvidence = { state: "ready", picker, inspection, layoutPreview, removalPreview, error: "" };
    } catch (error) {
      if (revision !== this.canonicalAuthoringEvidenceRevision || !this.root.isConnected) return;
      this.canonicalAuthoringEvidence = { state: "failed", picker: null, inspection: null, layoutPreview: null, removalPreview: null, error: String(error?.message ?? error) };
      console.error("Veilrunner | Canonical authoring inspection failed", error);
    }
    this.#refreshTreeEditorInspector();
  }

  #canonicalGraphDraftMarkup() {
    const evidence = this.canonicalAuthoringEvidence;
    const graph = evidence?.layoutPreview?.preview?.beforeGraph;
    const record = this.#treeEditorSelectedRecords()[0];
    const nodeId = this.#canonicalGraphNodeId(record);
    const node = graph?.nodes?.find(entry => entry.id === nodeId);
    if (evidence?.state !== "ready" || !node) return "";
    const draft = this.canonicalGraphDraft;
    const targets = graph.nodes.filter(entry => entry.id !== nodeId);
    const incidentEdges = graph.edges.filter(edge => edge.sourceId === nodeId || edge.targetId === nodeId);
    return `<section class="vr-cc-canonical-graph-draft" aria-label="Graph edit previews"><details><summary>Move and connection previews</summary><fieldset ${this.canonicalReview.state.busy ? "disabled" : ""}>
      <p>Preview only. Review and separate authorization are required for any persistent write.</p>
      <label>Node X<input type="number" step="any" name="canonicalGraphDraft.x" value="${escape(draft.x === null ? node.position.x : draft.x)}" /></label>
      <label>Node Y<input type="number" step="any" name="canonicalGraphDraft.y" value="${escape(draft.y === null ? node.position.y : draft.y)}" /></label>
      <button type="button" class="vr-cc-btn" data-action="preview-canonical-layout">Preview move</button>
      <label>Connection target<select name="canonicalGraphDraft.targetId"><option value="">Choose a target</option>${targets.map(target => `<option value="${escape(target.id)}" ${draft.targetId === target.id ? "selected" : ""}>${escape(target.presentation?.label || target.id)}</option>`).join("")}</select></label>
      <button type="button" class="vr-cc-btn" data-action="preview-canonical-connection-add">Preview added connection</button>
      <small>Existing incident connections</small>${incidentEdges.length ? `<ul>${incidentEdges.map(edge => `<li><span>${escape(edge.id)}</span><button type="button" class="vr-cc-btn" data-action="preview-canonical-connection-remove" data-edge-id="${escape(edge.id)}">Preview disconnect</button></li>`).join("")}</ul>` : "<p>No incident connections.</p>"}
    </fieldset></details></section>`;
  }

  async #previewCanonicalGraphEdit(action, edgeId = "") {
    if (this.canonicalReview.state.busy) return;
    this.canonicalDraft.open = true;
    const graph = this.canonicalAuthoringEvidence?.layoutPreview?.preview?.beforeGraph;
    const nodeId = this.#canonicalGraphNodeId(this.#treeEditorSelectedRecords()[0]);
    const node = graph?.nodes?.find(entry => entry.id === nodeId);
    try {
      if (!node) throw new Error("Refresh canonical graph evidence before editing.");
      const built = action === "preview-canonical-layout" ? buildProgressionLayoutDraft({ page: this.treePage, positions: [{ nodeId, position: {
        x: this.canonicalGraphDraft.x === null ? node.position.x : this.canonicalGraphDraft.x,
        y: this.canonicalGraphDraft.y === null ? node.position.y : this.canonicalGraphDraft.y
      } }] }) : buildProgressionConnectionChangeDraft({ page: this.treePage, graph, sourceId: nodeId, targetId: this.canonicalGraphDraft.targetId, removeEdgeId: edgeId });
      if (!await this.canonicalReview.preview(built)) return;
    } catch (error) {
      this.canonicalReview.refuse(error);
    }
  ...
  #canonicalAuthoringEvidenceMarkup() {
    const evidence = this.canonicalAuthoringEvidence ?? { state: "unavailable" };
    if (evidence.state === "loading") return `<section class="vr-cc-canonical-evidence"><small>Canonical services</small><p><i class="fa-solid fa-spinner fa-spin"></i> Loading picker and graph evidence...</p></section>`;
    if (evidence.state === "failed") return `<section class="vr-cc-canonical-evidence error"><small>Canonical services</small><strong>Inspection unavailable</strong><p>${escape(evidence.error)}</p></section>`;
    if (evidence.state !== "ready") return `<section class="vr-cc-canonical-evidence"><small>Canonical services</small><p>Waiting for the registered runtime.</p></section>`;
    const picker = evidence.picker ?? { items: [], total: 0, excludedPlaced: [] };
    const inspection = evidence.inspection;
    const layoutPreview = evidence.layoutPreview;
    const removalPreview = evidence.removalPreview;
    const candidates = (picker.items ?? []).slice(0, 8);
    const removalEvidence = removalPreview
      ? removalPreview.state === "previewed"
        ? `<p class="vr-cc-field-help"><i class="fa-solid fa-circle-check"></i> Semantic removal preview: ${escape(removalPreview.status ?? "validated")}; definition preserved.</p>`
        : `<p class="vr-cc-field-help"><i class="fa-solid fa-shield-halved"></i> Semantic removal preview: ${escape(removalPreview.state ?? "refused")} (${escape(removalPreview.code ?? "guarded")}). ${escape(removalPreview.issues?.[0]?.message ?? removalPreview.message ?? "")}</p>`
      : "";
    const selected = this.#treeEditorSelectedRecords()[0] ?? null;
    const parentSelected = Boolean(this.#canonicalGraphNodeId(selected));
    const removalAction = parentSelected && selected.kind !== "root" ? `<button type="button" class="vr-cc-btn" data-action="preview-canonical-removal"><i class="fa-solid fa-link-slash"></i><span>Preview graph-only removal</span></button>` : "";
    const pickerMarkup = candidates.length
      ? `<ul>${candidates.map(item => `<li><span>${escape(item.name)}</span><small>${escape(item.type)} - ${escape(item.definitionId)}</small><button type="button" class="vr-cc-btn" data-action="preview-canonical-placement" data-definition-id="${escape(item.definitionId)}" ${parentSelected ? "" : "disabled"}>Preview placement</button></li>`).join("")}</ul>`
      : `<p class="vr-cc-field-help">No canonical definitions match this page and filter without an existing placement.</p>`;
    return `<section class="vr-cc-canonical-evidence ready"><small>Canonical services</small><strong>${inspection ? `Graph node: ${escape(inspection.status ?? "resolved")}` : "Graph inventory ready"}</strong><p>${number(picker.total)} unplaced definition${number(picker.total) === 1 ? "" : "s"}; ${(picker.excludedPlaced ?? []).length} already placed.</p>${layoutPreview ? `<p class="vr-cc-field-help"><i class="fa-solid fa-circle-check"></i> Semantic layout preview: ${escape(layoutPreview.status ?? "validated")}.</p>` : ""}${removalEvidence}${removalAction}${pickerMarkup}<p class="vr-cc-field-help"><i class="fa-solid fa-lock"></i> Previews are read-only. Persistent writes remain disabled; definition deletion is unavailable.</p></section>`;
  }
  ...
  #treeEditorInspector() {
    ...
    if (this.treeEditorTool === "connect") {
      ...
      content = `<div class="vr-cc-editor-inspector-heading"><small>Inspect mode</small><h2>Canonical canvas ready</h2><p>Click a node to load its canonical graph and definition evidence.</p></div>`;
    }
    const gestureHelp = this.canonicalGestureTool === "inspect" ? "" : `<p class="vr-cc-canonical-gesture-help" role="status">${this.canonicalGestureTool === "move" ? "Drag a node to preview its position." : "Drag from a source socket to a target socket to preview a prerequisite connection."} Select a node first to load graph evidence. Release to validate; Escape cancels. The canonical canvas stays unchanged.</p>`;
    return `<aside class="vr-cc-tree-editor-inspector ${this.treeEditorInspectorOpen ? "open" : ""}" aria-label="Tree editor inspector"><button type="button" class="vr-cc-inspector-drawer-handle" data-action="toggle-tree-editor-inspector" aria-expanded="${this.treeEditorInspectorOpen}"><i class="fa-solid fa-chevron-up"></i><span>Inspector</span></button><div class="vr-cc-tree-editor-inspector-scroll">${gestureHelp}${this.#canonicalAuthoringEvidenceMarkup()}${this.#canonicalGraphDraftMarkup()}${this.#canonicalDraftMarkup()}${content}</div></aside>`;
  }
  ...
  #talentCanvasV2() {
    ...
    const controls = `<footer class="vr-cc-tree-viewport-controls">${this.isTreeEditor ? canvasSwitch : `<button type="button" data-action="fit-tree-view"><i class="fa-solid fa-expand"></i><span>Fit to View</span></button>${canvasSwitch}`}<div class="vr-cc-tree-zoom-controls"><button type="button" data-action="tree-zoom-out" aria-label="Zoom out"><i class="fa-solid fa-minus"></i></button><small>${Math.round(TREE_ZOOM.minimum * 100)}%</small><input type="range" name="treeZoom" min="${TREE_ZOOM.minimum}" max="${TREE_ZOOM.maximum}" step="${TREE_ZOOM.step}" value="${this.treeZoom}" aria-label="Tree zoom from ${Math.round(TREE_ZOOM.minimum * 100)}% to ${Math.round(TREE_ZOOM.maximum * 100)}%" /><small>${Math.round(TREE_ZOOM.maximum * 100)}%</small><button type="button" data-action="tree-zoom-in" aria-label="Zoom in"><i class="fa-solid fa-plus"></i></button><output>${Math.round(this.treeZoom * 100)}%</output></div></footer>`;
  ...
  async #saveTreeGuidance() {
    if (this.isTreeEditor && !this.canonicalCanvasMutationsEnabled) return;
  ...
  #rootMarkup(root, investment = { total: 0, talent: 0, skill: 0 }) {
    ...
    const drag = this.isTreeEditor && this.canonicalCanvasMutationsEnabled ? `draggable="true" data-tree-drag="root:${escape(root.id)}"` : "";
  ...
  #treePathsMarkup(records) {
    ...
    const groupedDrawRecords = new Map();
    for (const record of consolidated) {
      const source = this.#connectionSelection(record.source, record.target);
      const key = [source.sourceKind, source.sourceId, record.state, record.classes, record.line.color, record.line.pattern, record.line.css, record.line.route].join("|");
      const grouped = groupedDrawRecords.get(key);
      if (grouped) grouped.path += ` ${record.path}`;
      else groupedDrawRecords.set(key, { ...record });
    }
    // The editor preserves logical paths for individual hit targets. Chargen paints one compound path per source/style.
    const drawRecords = this.isTreeEditor ? consolidated : [...groupedDrawRecords.values()];
    this.treePulsePlan = null;
    if (!this.isTreeEditor) {
      // Animate logical connections, never paint fragments. Fragment splitting is only
      // for rendering shared geometry; using it here created duplicate beads at waypoints.
      const activeRoutes = logical.filter(record => record.state === "active");
      const pulseSpeed = 150;
      const routePhases = treeRouteSourcePhases(activeRoutes.map(record => ({
        sourceId: this.#connectionSelection(record.source, record.target).sourceId,
        targetId: this.#connectionSelection(record.source, record.target).targetId,
        length: record.pulseLength
      })));
      // Pulses are decorative. Cap them so a fully purchased large tree never
      // becomes a perpetual SVG animation workload.
      const pulseBranches = this.#treePulseBranches(activeRoutes).slice(0, 50);
      const cycleMs = Math.max(2000, ...pulseBranches.map(record => (((routePhases.get(this.#connectionSelection(record.source, record.target).sourceId) ?? 0) + record.pulseLength) / pulseSpeed) * 1000)) + 800;
      this.treePulsePlan = {
        cycleMs,
        entries: pulseBranches.map(record => ({
          delayMs: ((routePhases.get(this.#connectionSelection(record.source, record.target).sourceId) ?? 0) / pulseSpeed) * 1000,
          durationMs: Math.max(120, ((Number(record.pulseLength) || 240) / pulseSpeed) * 1000),
          path: record.path,
    ...
    const pathMarkup = record => {
      ...
      const metadata = this.isTreeEditor ? this.#treeLineMetadata(record) : `data-tree-draw-source="${escape(this.#connectionSelection(record.source, record.target).sourceId)}" data-line-route="${escape(record.line.route)}"`;
    ...
    const casingMarkup = record => `<path class="vr-cc-tree-link crossing-casing ${record.state}" style="${record.line.css}" ${this.isTreeEditor ? this.#treeLineMetadata(record) : ""} d="${record.path}" aria-hidden="true" />`;
    ...
    const layers = [
      ...(this.isTreeEditor ? [`<g class="vr-cc-tree-layer vr-cc-tree-casing-layer">${drawRecords.map(casingMarkup).join("")}</g>`] : []),
      ...TREE_PATH_STATE_ORDER.map(state => `<g class="vr-cc-tree-layer vr-cc-tree-${state}-layer">${drawRecords.filter(record => record.state === state).map(pathMarkup).join("")}</g>`),
      `<g class="vr-cc-tree-layer vr-cc-tree-pulse-layer"></g>`
    ];
    if (this.isTreeEditor && this.canonicalCanvasMutationsEnabled && game.user.isGM) layers.push(`<g class="vr-cc-tree-layer vr-cc-tree-hit-layer">${logical.map(hitboxMarkup).join("")}</g>`);
    return layers.join("");
  }

  #schoolMarkup(school, showSpells = false) {
    const drag = this.isTreeEditor && this.canonicalCanvasMutationsEnabled ? `draggable="true" data-tree-drag="school:${escape(school.id)}"` : "";
  ...
  #practiceMarkup(school, practice, showSpells = false) {
    ...
    // Canonical inspection must not inherit the actor's purchase availability.
    const available = this.isTreeEditor || (this.#schoolAccess(school).unlocked && requirementsMet && levelMet);
    const drag = this.isTreeEditor && this.canonicalCanvasMutationsEnabled ? `draggable="true" data-tree-drag="practice:${escape(school.id)}:${escape(practice.id)}"` : "";
  ...
  #spellRow(school, practice, spell, practicePurchased, color) {
    ...
    const available = this.isTreeEditor || (practicePurchased && requirementsMet && levelMet && rank < maximum);
    ...
    const drag = this.isTreeEditor && this.canonicalCanvasMutationsEnabled ? `draggable="true" data-tree-drag="spell:${escape(school.id)}:${escape(practice.id)}:${escape(spell.id)}"` : "";
  ...
  async #saveAuthoredTreeNodeV2() {
    throw new Error(AUTHORING_UNAVAILABLE);
  }
  ...
  #onTreeWheel(event) {
    if (this.canonicalGesture.state) { event.preventDefault(); return; }
  ...
  #applyTreeZoom(value, { clientX, clientY } = {}) {
    ...
    const minimumZoom = next <= TREE_ZOOM.minimum + .001;
    ...
    world.classList.add("tree-zooming");
    ...
    if (minimumZoom) {
      // Do not clear the existing origin first: that causes a visible one-frame
      // jump to the canvas origin before the centered minimum-zoom placement.
    } else {
      // A prior minimum zoom may have introduced a layout origin. Convert the
      // current scroll back to world space before returning to anchored zoom.
      const previousOriginX = Number.parseFloat(world.style.left) || 0;
      const previousOriginY = Number.parseFloat(world.style.top) || 0;
      world.style.left = "0px";
      world.style.top = "0px";
      const viewport = anchoredTreeViewport({ left: board.scrollLeft - previousOriginX, top: board.scrollTop - previousOriginY, anchorX, anchorY, fromZoom: previous, toZoom: next });
      board.scrollLeft = viewport.left;
      board.scrollTop = viewport.top;
    }
    this.#centerTreeWhenNeeded(board, world, frame);
    this.treeViewport[this.#treeViewportKey()] = { left: board.scrollLeft, top: board.scrollTop };
    ...
    // Preserve the existing frame while the wheel is moving, then repaint the
    // underlay at device-pixel-ratio resolution once the zoom settles.
    this.#renderTreeWebGL(world, { zoom: next, geometry: false, settled: false });
    if (this.treeZoomSettleTimer) window.clearTimeout(this.treeZoomSettleTimer);
    this.treeZoomSettleTimer = window.setTimeout(() => {
      this.treeZoomSettleTimer = 0;
      if (!world.isConnected) return;
      world.classList.remove("tree-zooming");
      this.#renderTreeWebGL(world, { zoom: this.treeZoom, geometry: false, settled: true });
    }, 110);
    this.#queueTreeViewportDetail(board);
  }

  #centerTreeWhenNeeded(board, world, frame) {
    if (!board || !world || !frame) return;
    const nodes = [...world.querySelectorAll("[data-tree-node-id]")];
    if (!nodes.length) return;
    const left = Math.min(...nodes.map(node => Number.parseFloat(node.style.left) - node.offsetWidth / 2));
    const right = Math.max(...nodes.map(node => Number.parseFloat(node.style.left) + node.offsetWidth / 2));
    const top = Math.min(...nodes.map(node => Number.parseFloat(node.style.top) - node.offsetHeight / 2));
    const bottom = Math.max(...nodes.map(node => Number.parseFloat(node.style.top) + node.offsetHeight / 2));
    const desiredLeft = ((left + right) / 2) * this.treeZoom - board.clientWidth / 2;
    const desiredTop = ((top + bottom) / 2) * this.treeZoom - board.clientHeight / 2;
    const offsetX = Math.max(0, -desiredLeft);
    const offsetY = Math.max(0, -desiredTop);
    world.style.left = `${offsetX}px`;
    world.style.top = `${offsetY}px`;
    frame.style.width = `${number(world.dataset.worldWidth, TREE_CANVAS_WIDTH) * this.treeZoom + offsetX}px`;
    frame.style.height = `${number(world.dataset.worldHeight, TREE_CANVAS_HEIGHT) * this.treeZoom + offsetY}px`;
    // Only replace the anchored scroll on an axis where the node bounds would
    // otherwise need negative scroll to be centered.
    if (offsetX) board.scrollLeft = 0;
    if (offsetY) board.scrollTop = 0;
  }

  #queueTreeViewportDetail(board = this.root.querySelector(".vr-cc-tree-board")) {
    if (this.isTreeEditor || this.treeViewportDetailFrame || !board) return;
    this.treeViewportDetailFrame = requestAnimationFrame(() => {
      this.treeViewportDetailFrame = 0;
      const world = board.isConnected ? board.querySelector(".vr-cc-tree-world") : null;
      if (!world) return;
      if (!world.dataset.treePerformanceMode) {
        const nodeCount = world.querySelectorAll("[data-tree-node-id]").length;
        const linkCount = world.querySelectorAll(".vr-cc-tree-link:not(.crossing-casing)").length;
        // Shadows and SVG filter regions scale much worse than path geometry.
        // Keep the rich treatment for normal trees, but use the same state
        // colors with cheaper paint for catalog-scale trees.
        world.classList.toggle("tree-performance-mode", nodeCount > 160 || linkCount > 320);
        world.dataset.treePerformanceMode = "checked";
      }
      const compact = this.treeZoom <= .7;
      const margin = 280 / Math.max(.1, this.treeZoom);
      const originX = Number.parseFloat(world.style.left) || 0;
      const originY = Number.parseFloat(world.style.top) || 0;
      const left = (board.scrollLeft - originX) / this.treeZoom - margin;
      const top = (board.scrollTop - originY) / this.treeZoom - margin;
      const right = (board.scrollLeft + board.clientWidth - originX) / this.treeZoom + margin;
      const bottom = (board.scrollTop + board.clientHeight - originY) / this.treeZoom + margin;
      world.classList.toggle("tree-low-detail", compact);
      for (const node of world.querySelectorAll("[data-tree-node-id]")) {
        ...
        const outside = !Number.isFinite(x) || !Number.isFinite(y) || x < left || x > right || y < top || y > bottom;
        node.classList.toggle("vr-cc-tree-node-simplified", compact && outside);
      }
    });
  }
  ...
  #onKeyDown(event) {
    ...
    if (!this.canonicalCanvasMutationsEnabled) {
      if (event.key === "Escape") {
        event.preventDefault();
        if (this.#cancelCanonicalGesture()) this.#refreshTreeEditorInspector();
        else if (this.canonicalGestureTool !== "inspect") {
          this.canonicalGestureTool = "inspect";
          this.canonicalReview.invalidate();
          this.#redrawTreePreservingViewport();
        } else this.close({ renderSheet: false });
      }
      return;
    }
  ...
  #onTreeDoubleClick(event) {
    if (!this.isTreeEditor || !this.canonicalCanvasMutationsEnabled || !game.user.isGM) return;
    if (this.treeEditorTool === "connect") {
  ...
  #canonicalPointerRecord(element) {
    const id = element?.dataset.treeSelect ?? element?.dataset.treeRoot ?? element?.dataset.treeSchool ?? element?.dataset.treePracticeCard ?? element?.dataset.treeRank;
    const kind = element?.dataset.treeSelectKind ?? (element?.dataset.treeRoot ? "root" : element?.dataset.treeSchool ? "school" : element?.dataset.treePracticeCard ? "practice" : "spell");
    return this.#treeCatalogRecords().find(record => record.id === id && record.kind === kind);
  }

  #cancelCanonicalGesture() {
    const pointerId = this.canonicalGesture.state?.pointerId;
    const active = this.canonicalGesture.cancel();
    const visual = this.canonicalGestureVisual;
    this.canonicalGestureVisual = null;
    visual?.ghost.remove();
    visual?.label.remove();
    visual?.socket?.classList.remove("drag-source");
    visual?.world.querySelectorAll(".drop-target").forEach(socket => socket.classList.remove("drop-target"));
    if (pointerId !== undefined && this.root?.hasPointerCapture?.(pointerId)) this.root.releasePointerCapture(pointerId);
    if (active) this.canonicalReview.invalidate();
    return active;
  }

  #onCanonicalPointerStart(event) {
    if (!this.isTreeEditor || !this.authoringSession || !game.user.isGM || this.canonicalGestureTool === "inspect") return false;
    if (this.canonicalReview.state.busy || this.canonicalGesture.state) return true;
    const socket = event.target.closest?.("[data-tree-socket]");
    const target = this.canonicalGestureTool === "connect" ? socket?.closest(".vr-cc-node-socket-layer") : event.target.closest?.("[data-tree-select]");
    if (!target) return false;
    const record = this.#canonicalPointerRecord(target);
    const world = target.closest(".vr-cc-tree-world");
    if (!record || !world) return false;
    const graph = this.canonicalAuthoringEvidence.state === "ready" ? this.canonicalAuthoringEvidence.layoutPreview?.preview?.beforeGraph : null;
    if (!graph) {
    ...
    try {
      this.canonicalReview.invalidate();
      this.canonicalGesture.begin({ mode: this.canonicalGestureTool, page: this.treePage, graph,
        nodeId: this.#canonicalGraphNodeId(record), pointerId: event.pointerId,
        point: this.#treePointerPosition(event, world), sourceAnchor: socket?.dataset.treeSocket });
      const connecting = this.canonicalGestureTool === "connect";
      const ghost = connecting ? document.createElementNS("http://www.w3.org/2000/svg", "path") : document.createElement("div");
      const label = document.createElement("output");
      label.className = "vr-cc-canonical-gesture-label";
      if (connecting) {
        ghost.setAttribute("class", "vr-cc-tree-link connection-preview vr-cc-canonical-gesture-link");
        world.querySelector(".vr-cc-tree-links").append(ghost);
      } else {
        ghost.className = "vr-cc-canonical-move-ghost";
        ghost.style.width = `${target.offsetWidth}px`;
        ghost.style.height = `${target.offsetHeight}px`;
        ghost.textContent = record.name;
        world.append(ghost);
      }
      world.append(label);
      socket?.classList.add("drag-source");
      this.canonicalGestureVisual = { ghost, label, socket, world, record };
      this.#refreshTreeEditorInspector();
      this.root.focus({ preventScroll: true });
      this.root.setPointerCapture?.(event.pointerId);
      this.#onCanonicalPointerMove(event);
      event.preventDefault();
    } catch (error) {
      this.#cancelCanonicalGesture();
      this.canonicalDraft.open = true;
  ...
  #onCanonicalPointerMove(event) {
    const state = this.canonicalGesture.state;
    const visual = this.canonicalGestureVisual;
    if (!state || !visual || state.pointerId !== event.pointerId) return;
    const point = this.#treePointerPosition(event, visual.world);
    const position = this.canonicalGesture.move({ pointerId: event.pointerId, point });
    if (!position) return;
    if (state.mode === "move") {
      visual.position = this.#clampTreeNodeCenter(visual.record.node, visual.record.kind, position.x, position.y);
      visual.ghost.style.left = `${visual.position.x}px`;
      visual.ghost.style.top = `${visual.position.y}px`;
      visual.label.textContent = `Move preview: ${visual.position.x}, ${visual.position.y}`;
    } else {
      visual.ghost.setAttribute("d", this.#treePointPath(state.start, point, { route: "linear" }));
      visual.world.querySelectorAll(".drop-target").forEach(socket => socket.classList.remove("drop-target"));
      const socket = document.elementsFromPoint(event.clientX, event.clientY).find(item => item.matches?.("[data-tree-socket]") && visual.world.contains(item));
      socket?.classList.add("drop-target");
      visual.label.textContent = "Connection preview: release on a target socket";
    }
    visual.label.style.left = `${(visual.position ?? point).x + 20}px`;
    visual.label.style.top = `${(visual.position ?? point).y - 35}px`;
  }

  async #onCanonicalPointerEnd(event) {
    const state = this.canonicalGesture.state;
    if (!state || state.pointerId !== event.pointerId) return;
    const visual = this.canonicalGestureVisual;
    // Suppress the synthetic click so it cannot select a node and clear the new review.
    ...
    try {
      this.#onCanonicalPointerMove(event);
      const socket = document.elementsFromPoint(event.clientX, event.clientY).find(item => item.matches?.("[data-tree-socket]") && visual.world.contains(item));
      const target = this.#canonicalPointerRecord(socket?.closest(".vr-cc-node-socket-layer"));
      const moved = this.canonicalGesture.state.moved;
      const built = this.canonicalGesture.finish({ pointerId: event.pointerId, point: this.#treePointerPosition(event, visual.world),
        position: visual.position, targetId: this.#canonicalGraphNodeId(target), targetAnchor: socket?.dataset.treeSocket });
      this.#cancelCanonicalGesture();
      if (this.root.hasPointerCapture?.(event.pointerId)) this.root.releasePointerCapture(event.pointerId);
      if (!built) {
        if (!moved) {
          this.treeFocus = { id: visual.record.id, kind: visual.record.kind };
          void this.#refreshCanonicalAuthoringEvidence();
        }
        return;
      }
      this.canonicalDraft.open = true;
      this.treeEditorInspectorOpen = true;
      const pending = this.canonicalReview.preview(built);
      this.#refreshTreeEditorInspector();
      if (!await pending || !this.root.isConnected) return;
    } catch (error) {
      this.#cancelCanonicalGesture();
      if (this.root.hasPointerCapture?.(event.pointerId)) this.root.releasePointerCapture(event.pointerId);
      this.canonicalDraft.open = true;
  ...
  #onTreePanStart(event) {
    ...
    if (!scroller) return;
    if (this.#onCanonicalPointerStart(event)) return;
    const waypoint = event.target.closest?.("[data-tree-waypoint]");
    if (this.canonicalCanvasMutationsEnabled && game.user.isGM && this.connectionTool.active && waypoint) {
    ...
    if (this.canonicalCanvasMutationsEnabled && game.user.isGM && this.connectionTool.active && connectionHitbox) {
    ...
    const socket = event.target.closest?.("[data-tree-socket]");
    if (this.canonicalCanvasMutationsEnabled && game.user.isGM && this.connectionTool.active && socket) {
    ...
    const dragTarget = this.canonicalCanvasMutationsEnabled && game.user.isGM && this.nodeMoveMode && !this.connectionTool.active ? event.target.closest?.("[data-tree-drag]") : null;
    if (dragTarget) {
  ...
  #onTreePanMove(event) {
    if (this.canonicalGesture.state) return this.#onCanonicalPointerMove(event);
    if (this.personaDrag) {
  ...
  async #onTreePanEnd(event) {
    if (this.canonicalGesture.state) return this.#onCanonicalPointerEnd(event);
    if (this.personaDrag) {
    ...
    if (this.treeNodeDrag) {
      ...
      if (drag.moved) {
        ...
        this.treeRingTool.layout = null;
        this.#refreshTreeEditorNodeSelection();
        if (this.selectedTreeNodes.size > 1) {
          this.treeEditorInspectorOpen = true;
          this.#redrawTreePreservingViewport();
        } else this.#refreshTreeEditorInspector();
      }
      return;
    }
    if (!this.treePan) return;
  ...
  #fitTreeToView() {
    ...
    const lefts = nodes.map(node => Number.parseFloat(node.style.left) - node.offsetWidth / 2);
    const rights = nodes.map(node => Number.parseFloat(node.style.left) + node.offsetWidth / 2);
    const tops = nodes.map(node => Number.parseFloat(node.style.top) - node.offsetHeight / 2);
    const bottoms = nodes.map(node => Number.parseFloat(node.style.top) + node.offsetHeight / 2);
    ...
    if (this.treeZoom <= TREE_ZOOM.minimum + .001) this.#centerTreeWhenNeeded(board, board.querySelector(".vr-cc-tree-world"), board.querySelector(".vr-cc-tree-world-frame"));
    else {
  ...
  #buyAllMagicSpellsForTesting() {
    if (!game.user.isGM) return;
    const branches = new Set(this.state.talentTree?.branches ?? []);
    const leaves = new Map((this.state.talentTree?.leaves ?? []).map(entry => [entry.id, { ...entry }]));
    branches.add(MAGIC_ACCESS_BRANCH_ID);
    for (const school of talentTreePage("magic")) {
      branches.add(schoolAccessBranchId(school.id));
      for (const practice of school.practices ?? []) {
        branches.add(practice.id);
        for (const spell of practice.spells ?? []) leaves.set(spell.id, { id: spell.id, rank: Math.max(1, number(spell.maxRank, 1)) });
      }
    }
    const next = { branches: [...branches], leaves: [...leaves.values()] };
    if (JSON.stringify(next) === JSON.stringify(this.state.talentTree ?? {})) return ui.notifications.info("All Magic Spells are already purchased for this test draft.");
    this.#pushTreeUndo("test purchase of all Magic Spells");
    this.state.talentTree = next;
    this.treePurchasedOnly = false;
    this.#draw();
    ui.notifications.info("Testing: all Magic Practices and Spells are now purchased at maximum rank.");
  }
  ...
  async #purchaseMagicAccess() {
    ...
    this.#refreshTalentTreePurchase(talentTreeRoot("magic").id, "root");
    return true;
  }
  ...
  async #purchaseSchoolAccess(schoolId) {
    ...
    this.#refreshTalentTreePurchase(schoolId, "school");
    return true;
  }
  ...
  async #onClick(event) {
    ...
    if (this.isTreeEditor && this.canonicalCanvasMutationsEnabled && this.connectionTool.active && connectionPath && game.user.isGM) {
    ...
    if (this.isTreeEditor && this.canonicalCanvasMutationsEnabled && this.connectionTool.active && connectionNode && game.user.isGM) {
    ...
    if (treeSelection) {
      if (this.isTreeEditor && this.canonicalReview.state.busy) return;
      this.treeHelpOpen = false;
      this.treeFocus = { id: treeSelection.dataset.treeSelect, kind: treeSelection.dataset.treeSelectKind ?? "" };
      if (this.isTreeEditor) {
        // Select mode is for inspection and multi-selection. Move mode keeps
        // drag-to-move behaviour, but Shift-click must work in either mode so
        // alignment tools are always reachable.
        const selectionKey = treeSelection.dataset.treeDrag;
        if (selectionKey && this.treeEditorTool !== "connect") {
          if (event.shiftKey) {
            if (this.selectedTreeNodes.has(selectionKey)) this.selectedTreeNodes.delete(selectionKey);
            else this.selectedTreeNodes.add(selectionKey);
          } else {
            this.selectedTreeNodes.clear();
            this.selectedTreeNodes.add(selectionKey);
          }
        ...
        if (!selectionKey) this.#refreshTreeEditorInspector();
        void this.#refreshCanonicalAuthoringEvidence();
      } else this.#refreshTalentTreeInspector();
      return;
    }
    ...
    if (this.isTreeEditor && !this.canonicalCanvasMutationsEnabled && LEGACY_TREE_EDITOR_MUTATION_ACTIONS.has(action)) return;
    ...
    if (action === "open-tree-editor") {
      ui.notifications.warn(AUTHORING_UNAVAILABLE);
      return;
    }
    if (action === "test-buy-all-spells") {
      this.#buyAllMagicSpellsForTesting();
      return;
    }
    ...
    if (action === "set-canonical-gesture-tool") {
      if (!this.isTreeEditor || !this.authoringSession || !game.user.isGM || this.canonicalReview.state.busy) return;
      this.#cancelCanonicalGesture();
      this.canonicalReview.invalidate();
      this.canonicalGestureTool = ["move", "connect"].includes(button.dataset.canonicalTool) ? button.dataset.canonicalTool : "inspect";
      this.#redrawTreePreservingViewport();
      return;
    }
    ...
    if (action === "set-tree-tool") {
      if (!this.canonicalCanvasMutationsEnabled) return;
      this.#setTreeEditorTool(button.dataset.treeTool);
      return;
    }
    if (action === "refresh-canonical-authoring") {
      void this.#refreshCanonicalAuthoringEvidence();
      return;
    }
    if (action === "toggle-canonical-draft") {
      if (this.canonicalReview.state.busy) return;
      this.canonicalReview.invalidate();
      this.canonicalDraft.open = !this.canonicalDraft.open;
      this.treeEditorInspectorOpen = true;
      this.#redrawTreePreservingViewport();
      return;
    }
    if (action === "clear-canonical-draft") {
      if (!this.canonicalReview.invalidate()) return;
      this.canonicalDraft = { open: true, type: this.treePage === "magic" ? "spell" : "skill", name: "", description: "" };
      this.#refreshTreeEditorInspector();
      return;
    }
    if (action === "preview-canonical-draft") {
      await this.#previewCanonicalDraft();
      return;
    }
    if (action === "preview-canonical-placement" || action === "preview-canonical-removal") {
      await this.#previewCanonicalPlacementOrRemoval({ definitionId: button.dataset.definitionId, remove: action === "preview-canonical-removal" });
      return;
    }
    if (["preview-canonical-layout", "preview-canonical-connection-add", "preview-canonical-connection-remove"].includes(action)) {
      await this.#previewCanonicalGraphEdit(action, button.dataset.edgeId);
      return;
    }
    if (action === "review-canonical-draft") {
      this.canonicalReview.review();
      this.#refreshTreeEditorInspector();
      return;
    }
    if (action === "cancel-canonical-draft-confirmation") {
      this.canonicalReview.back();
      this.#refreshTreeEditorInspector();
      return;
    }
    if (action === "commit-canonical-draft") {
      const pending = this.canonicalReview.commit();
      this.#refreshTreeEditorInspector();
      await pending;
      this.#refreshTreeEditorInspector();
      this.root.querySelector(".vr-cc-canonical-draft-result[tabindex]")?.focus({ preventScroll: false });
    ...
    if (button.dataset.treePage) {
      ...
      this.canonicalDraft = { ...this.canonicalDraft, type: this.treePage === "magic" ? "spell" : "skill" };
      this.treeHelpOpen = false;
      this.openPracticeId = null;
      this.treeFocus = null;
      this.treeSearch = "";
      ...
      if (this.isTreeEditor) void this.#refreshCanonicalAuthoringEvidence();
      return;
    }
    ...
    if (button.dataset.treePractice) {
      ...
      this.#refreshTalentTreePurchase(entry.practice.id, "practice");
      return;
    }
    if (button.dataset.treeRank) {
      ...
      this.#refreshTalentTreePurchase(spell.id, "spell");
      return;
    }
    ...
    if (button.dataset.referenceChoicePath) {
      ...
      this.state.references[path + "DefinitionId"] = "";
      const reference = this.#selectedReference(path);
      this.state.references[path + "DefinitionId"] = reference?.definitionId ?? "";
  ...
  #onInput(event) {
    ...
    if (input.name?.startsWith("canonicalGraphDraft.")) {
      if (this.canonicalReview.state.busy) return;
      const field = input.name.slice("canonicalGraphDraft.".length);
      if (["x", "y", "targetId"].includes(field)) this.canonicalGraphDraft[field] = input.value;
      this.canonicalReview.invalidate();
      const evidence = this.root.querySelector("[data-canonical-review-evidence]");
      if (evidence) evidence.innerHTML = `<p class="vr-cc-field-help" role="status" aria-live="polite">Graph draft changed. Validate a fresh preview before review.</p>`;
      return;
    }
    if (input.name?.startsWith("canonicalDraft.")) {
      if (this.canonicalReview.state.busy) return;
      const field = input.name.slice("canonicalDraft.".length);
      if (field === "acknowledged" && input instanceof HTMLInputElement) {
        this.canonicalReview.acknowledge(input.checked);
        this.#refreshTreeEditorInspector();
        return;
      }
      if (["type", "name", "description"].includes(field)) this.canonicalDraft[field] = input.value;
      this.canonicalReview.invalidate();
      // Remove stale review controls without replacing the focused text input.
      const evidence = this.root.querySelector("[data-canonical-review-evidence]");
      if (evidence) evidence.innerHTML = `<p class="vr-cc-field-help" role="status" aria-live="polite">Draft changed. Validate a fresh preview before review.</p>`;
      if (field === "type" && event.type === "change") this.#refreshTreeEditorInspector();
      return;
    }
  ...
  async #confirm() {
    ...
    if (!treeItemValidation.valid) {
      ui.notifications.error(`Character generation cannot finish until its canonical Spell and Skill definitions are available: ${treeItemValidation.errors.join(" ")}`);
      return;
    }
    const update = {
      name,
  ...
  async #confirmLevelUp() {
    ...
    const treeItemValidation = await validateTalentTreeItemSources(this.state.talentTree, talentTreeCatalog(), this.actor);
    if (!treeItemValidation.valid) {
      ui.notifications.error(`Level-up cannot finish until its canonical Spell and Skill definitions are available: ${treeItemValidation.errors.join(" ")}`);
      return;
    }
