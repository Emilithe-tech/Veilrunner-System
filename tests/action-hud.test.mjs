...
test("drawer swaps outside combat clear displaced slots and only temporary grants", async () => {
  const old = { id: "old", type: "weapon", system: { requiredSlots: ["mainHand", "offhand"], rules: [] } };
  const replacement = { id: "new", name: "New", type: "weapon", parent: { id: "hero" }, system: { requiredSlots: ["mainHand"], rules: [] } };
  const temporary = { id: "temporary", flags: { Veilrunner: { grantedBy: "old:grant", grantDuration: "while-equipped" } } };
  const permanent = { id: "permanent", flags: { Veilrunner: { grantedBy: "old:grant", grantDuration: "permanent" } } };
  const hero = actor({ items: collection([old, replacement, temporary, permanent]) });
  hero.system.equipment = { mainHand: old.id, offhand: old.id };
  const deleted = [];
  hero.deleteEmbeddedDocuments = async (type, ids) => { deleted.push(...ids); };
  const previousCombat = game.combat;
  try {
    // A prepared encounter must not charge actions or require an active turn.
    game.combat = { started: false, combatants: [{ actor: hero }] };
    assert.equal(await equipPhysicalItem(hero, replacement, { replaceOutsideCombat: true }), true);
    assert.equal(hero.system.equipment.mainHand, replacement.id);
    assert.equal(hero.system.equipment.offhand, "");
    assert.deepEqual(hero.system.equipmentAssignments, [{ itemId: replacement.id, slots: ["mainHand"] }]);
    assert.deepEqual(deleted, [temporary.id]);
    assert.equal(hero.items.get(old.id), old);
    assert.equal(await equipPhysicalItem(hero, replacement, { replaceOutsideCombat: true }), true);
    assert.deepEqual(deleted, [temporary.id], "repeated drops are a no-op");
  } finally { game.combat = previousCombat; }
});

test("drawer swaps cannot replace occupied slots during active combat", async () => {
  const replacement = { id: "new", name: "New", type: "weapon", parent: { id: "hero" }, system: { requiredSlots: ["mainHand"], rules: [] } };
  const hero = actor({ items: collection([replacement]) });
  hero.system.equipment = { mainHand: "old" };
  const previousCombat = game.combat;
  ...
  try {
    ...
    assert.equal(await equipPhysicalItem(hero, replacement, { replaceOutsideCombat: true }), undefined);
    assert.equal(hero.system.equipment.mainHand, "old");
  } finally { game.combat = previousCombat; globalThis.ui = previousUi; game.i18n = previousI18n; }
});
